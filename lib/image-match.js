...
/**
 * Auto-add threshold, in top1–top2 logit margin. The probe's softmax stays almost
 * uniform (≈0.01) even for a correct match, so `prob` is not a usable confidence —
 * the logit margin is. Above this we're confident enough to skip the picker.
 */
export const AUTO_MARGIN = 0.3;
 ...
 */
/**
 * Embed a photo with the frozen backbone. Returns the unit-norm feature vector
 * (shared by the matcher and the mug/not-mug gate) plus its raw norm.
 */
...
export async function matchMug(imageDataUrl, { topK = 5 } = {}) {
  ...
  return { candidates: candidates.slice(0, topK), autoMargin, embedding: toBase64(Float32Array.from(x)), vec: x, model: index.model, diag };
}
