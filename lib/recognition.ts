...
export async function recognizeMug(ai: AiMug, photoDataUrl: string, opts: { verify?: boolean } = {}): Promise<Recognition> {
  ...
  const candidates = catalogCandidates(query, 8);
  ...
  if (opts.verify !== false && geminiConfigured()) {
    ...
    if (refs.length) {
      try {
        ...
        // `match` is 1-based (0 = none); convert to a 0-based array index here.
        if (vr && vr.match >= 1 && vr.match <= refs.length && vr.confidence >= 0.5) {
          const vConf = clamp(vr.confidence);
          return { catalog: resolveCandidate(refs[vr.match - 1].entry), confidence: clamp(0.35 + 0.35 * vConf + 0.2 * aiConf, 0, 0.9), verified: true, reason: "verified" };
        }
  ...
  return { catalog: resolveCandidate(top.entry), confidence: clamp(0.3 + 0.4 * aiConf, 0, 0.7), verified: false, reason: "unverified" };
}
