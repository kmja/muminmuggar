...
/* -------------------------- back gesture ------------------------------- */
// Close the top-most dialog when the user swipes back from the screen edge
// (iOS) or presses back (Android). A sentinel history entry is pushed while a
// dialog is open so the gesture is caught; with no dialog open the gesture
// falls through to the browser default (usually "go back").
const DIALOG_STACK = [];
let popBound = false;
let suppressPops = 0;
function bindPop() {
  ...
  window.addEventListener("popstate", () => {
    ...
    const close = DIALOG_STACK[DIALOG_STACK.length - 1];
    if (close) close();
  });
}
export function useBackToClose(open, onClose) {
  ...
  useEffect(() => {
    ...
    let popped = false, pushed = false;
    try { window.history.pushState({ __dialog: true }, ""); pushed = true; } catch { /* ignore */ }
    const close = () => { popped = true; closeRef.current?.(); };
    DIALOG_STACK.push(close);
    bindPop();
    return () => {
      const i = DIALOG_STACK.lastIndexOf(close);
      if (i >= 0) DIALOG_STACK.splice(i, 1);
      if (!popped && pushed) {
        // Closed programmatically — drop our sentinel to keep history balanced.
        suppressPops += 1;
